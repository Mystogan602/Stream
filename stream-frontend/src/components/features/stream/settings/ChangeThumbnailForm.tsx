...
import { ConfirmModal } from '@/components/ui/elements/ConfirmModel';
...
export function ChangeThumbnailForm({ stream }: ChangeThumbnailFormProps) {
	...
	const { incrementVersion } = useThumbnailStore();
	const { user, refetch } = useCurrent();
	const thumbnailUrl = useThumbnailUrl(stream?.thumbnailUrl);
	...
	const form = useForm<TypeUploadFileSchema>({
		...
		values: {
			file: getMediaSource(stream?.thumbnailUrl!) || undefined
		}
	});
