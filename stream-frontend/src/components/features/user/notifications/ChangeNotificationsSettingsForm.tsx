...
export function ChangeNotificationsSettingsForm() {
	...
	const form = useForm<TypeChangeNotificationsSettingsSchema>({
		...
		defaultValues: {
			siteNotifications:
				user?.notificationSettings?.[0]?.siteNotifications ?? false,
			telegramNotifications:
				user?.notificationSettings?.[0]?.telegramNotifications ?? false
		}
	});
		...
		useChangeNotificationsSettingsMutation({
			onCompleted(data) {
				...
				if (data.changeNotificationsSettings.telegramAuthToken) {
					window.open(
						`https://t.me/mysstream_bot?start=${data.changeNotificationsSettings.telegramAuthToken}`,
						'_blank'
					);
				}
			},
